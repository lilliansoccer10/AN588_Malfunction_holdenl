...
```{r}
slope.one <- lm(MaxLongevity_m ~ Brain_Size_Species_Mean, data = d)
summary(slope.one)
slope.two <-  lm(log(MaxLongevity_m) ~ log(Brain_Size_Species_Mean), data = d)
summary(slope.two)
```
...
g <- g + geom_text() # I can't seem to plot it
g
